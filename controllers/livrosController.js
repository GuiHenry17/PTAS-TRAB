...
const buscarLivro = (req, res) => {
  const resultado = livrosCadastrados.filter(
    (livro) => livro.id == req.params.id
  );
...
const cadastrarLivro = (req, res) => {
  ...
  if (typeof notaAvaliacao !== "number" || notaAvaliacao < 0) {
    return res.status(400).json({
      error: true,
      message:
        "A nota de avaliação precisa ser um número maior ou igual a zero.",
    });
  }
...
const removerLivro = (req, res) => {
  ...
  const novaLista = livrosCadastrados.filter(
    (livro) => livro.id !== idConvertido
  );
